...
export default function QuizResult() {
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in space-y-6">
      {/* Hero result */}
      <div className={`card text-center border-2 ${isPassing ? 'border-green-500/30 bg-green-900/10' : 'border-red-500/30 bg-red-900/10'}`}>
        <div className="text-4xl mb-3">{isPassing ? '🏆' : '💪'}</div>
        <h1 className="text-3xl font-display font-bold text-white mb-1">
          {isPassing ? 'Well Done!' : 'Keep Practicing!'}
        </h1>
        <p className="text-gray-400 text-sm mb-6">{quizId?.title}</p>

        {/* Score circle */}
        <div className="flex justify-center mb-6">
          <div className="relative w-36 h-36">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 120 120">
              <circle cx="60" cy="60" r={circleR} fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="8" />
              <circle cx="60" cy="60" r={circleR} fill="none"
                stroke={isPassing ? '#22c55e' : '#ef4444'} strokeWidth="8"
                strokeDasharray={`${dash} ${circumference - dash}`}
                strokeLinecap="round" className="transition-all duration-1000" />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-3xl font-display font-bold text-white">{percentage}%</span>
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className="glass-dark rounded-xl p-3 text-center">
      ...
      <div className="flex gap-3">
        <Link to={`/quiz/${id}/leaderboard`} className="btn-secondary flex-1 py-3 text-center flex items-center justify-center gap-2">
          <FiAward /> Leaderboard
        </Link>
        <Link to="/quizzes" className="btn-primary flex-1 py-3 flex items-center justify-center gap-2">
        ...
        <div>
          ...
          <div className="space-y-4">
            {questionDetails.map((q, idx) => (
              <div key={q._id} className={`card border ${q.isCorrect ? 'border-green-500/20' : q.userAnswer === -1 ? 'border-gray-600/20' : 'border-red-500/20'}`}>
                <div className="flex items-start gap-3 mb-4">
                  <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0
                    ${q.isCorrect ? 'bg-green-500/20 text-green-400' : q.userAnswer === -1 ? 'bg-gray-600/20 text-gray-400' : 'bg-red-500/20 text-red-400'}`}>
                    {idx + 1}
                  </span>
                ...
                {q.explanation && (
                  <div className="mt-3 pl-10 p-3 bg-brand-600/10 rounded-lg border border-brand-500/20">
                    <p className="text-brand-300 text-xs"><strong>Explanation:</strong> {q.explanation}</p>
                  </div>
                )}
              </div>
            ))}
